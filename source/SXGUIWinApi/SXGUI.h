...
struct ISXGUIEdit : public virtual ISXGUITextual
{
	...
	virtual int Align() = 0;			//возвращает флаг выравнивания текста
	virtual bool Align(int align) = 0;	//устанавливает выравнивание
};
...
struct ISXGUIStatusBar : public virtual ISXGUIComponent
{
	...
	virtual bool GetTextParts(WORD pos, char* buf, int len) = 0;
...
#define SXGUI_DIALOG_FILE_OPEN 0
#define SXGUI_DIALOG_FILE_SAVE 1

namespace SXGUIDialogs
{
	SX_LIB_API void SelectFile(int type, char* path, char* name, const char* stdpath, const char* filter);
};
