...
namespace DirectX
{
	...
	void MakeXHeap(
		...
		_In_reads_(nVerts) const float3* positions, size_t nVerts)
	{
		...
		_Out_writes_(nVerts) uint32_t* pointRep)
	{
		...
		else
		{
			...
			float4 vepsilon = float4(epsilon * epsilon);
			...
			while(tail < nVerts)
			{
				...
				if(pointRep[tailIndex] == UNUSED32)
				{
					...
					float3 outer = positions[tailIndex];
					...
					for(uint32_t current = tail + 1; current < head; ++current)
					{
						...
						{
							float3 inner = positions[curIndex];

							float3 diff = SMVector3Length2(inner - outer);

							if(diff.x < vepsilon.x && diff.y < vepsilon.y)
							{
			...
			for(uint32_t point = 0; point < 3; ++point)
			{
				...
				if(current)
				{
					...
					while(current != nullptr)
					{
						if((current->v2 == vb) && (current->v1 == va))
						{
							float3 pB1 = positions[vb];
							float3 pB2 = positions[va];
							float3 pB3 = positions[vOther];

							float3 v12 = pB1 - pB2;
							float3 v13 = pB1 - pB3;

							float3 bnormal = SMVector3Normalize(SMVector3Cross(v12, v13));

							if(bestDiff == -2.f)
							{
								float3 pA1 = positions[found->v1];
								float3 pA2 = positions[found->v2];
								float3 pA3 = positions[found->vOther];

								v12 = pA1 - pA2;
								v13 = pA1 - pA3;

								float3 anormal = SMVector3Cross(v12, v13);

								bestDiff = SMVector3Dot(anormal, bnormal);
							}

							float3 pA1 = positions[current->v1];
							float3 pA2 = positions[current->v2];
							float3 pA3 = positions[current->vOther];

							v12 = pA1 - pA2;
							v13 = pA1 - pA3;

							float3 anormal = SMVector3Normalize(SMVector3Cross(v12, v13));

							float diff = SMVector3Dot(anormal, bnormal);
	...
	HRESULT GenerateAdjacencyAndPointReps(
		...
		const float3* positions, size_t nVerts,
		float epsilon,
