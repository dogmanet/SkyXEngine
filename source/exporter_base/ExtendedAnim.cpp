...
INT_PTR CALLBACK CExtendedAnim::DlgProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	CExtendedAnim *pThis = (CExtendedAnim*)GetWindowLongPtr(hWnd, GWLP_USERDATA);
	if(msg == WM_INITDIALOG)
	{
		SetWindowLongPtr(hWnd, GWLP_USERDATA, (LONG_PTR)lParam);
		pThis = (CExtendedAnim*)lParam;
		pThis->m_hDlg = hWnd;
	}
	if(pThis)
	{
...
INT_PTR CALLBACK CExtendedAnim::dlgProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		{
			m_hListWnd = GetDlgItem(m_hDlg, IDC_ANIM_LIST);
...
void CExtendedAnim::init(HWND hWndParent)
{
	assert(!m_hDlg);

	m_hDlg = CreateDialogParamA(g_hInstance, MAKEINTRESOURCE(IDD_OP_ANIM), hWndParent, DlgProc, (LPARAM)this);
}

void CExtendedAnim::show(int X, int Y, int cx, int cy)
{
	ShowWindow(m_hDlg, SW_SHOWNORMAL);
	SetWindowPos(m_hDlg, HWND_TOP, X, Y, cx, cy, 0);
}
void CExtendedAnim::hide()
{
	ShowWindow(m_hDlg, SW_HIDE);
}

void CExtendedAnim::loadDefaults()
{
	ListView_DeleteAllItems(m_hListWnd);
	
	char tmp[32];

	for(int i = 0, l = m_aSequences.size(); i < l; ++i)
	{
...
void CExtendedAnim::rollback()
{
	loadDefaults();
}

void CExtendedAnim::setActivityList(const Array<String> &list)
{
	m_asActivities = list;
}

void CExtendedAnim::addSequence(ModelSequence &ms)
{
	String sAct = "";
