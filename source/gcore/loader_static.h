...
struct DataStaticModel : public ISXDataStaticModel
{
	...
	void syncBuffers(bool bRecreate = false)
	{
		...
		m_pVertexBuffer = g_pDevice->createVertexBuffer(sizeof(vertex_static_ex)* m_uiAllVertexCount, GXBUFFER_USAGE_STATIC, m_pVertices);
		m_pRenderBuffer = g_pDevice->createRenderBuffer(1, &m_pVertexBuffer, SGCore_StaticModelGetDecl());

		m_pIndexBuffer = g_pDevice->createIndexBuffer(sizeof(UINT)* m_uiAllIndexCount, GXBUFFER_USAGE_STATIC, GXIT_UINT32, m_pIndices);
	}
