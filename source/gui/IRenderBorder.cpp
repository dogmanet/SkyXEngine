...
namespace GUI
{
	namespace DOM
	{
		namespace Render
		{
			...
			{
				SX_SAFE_RELEASE(m_pIndexBuffer);
				SX_SAFE_RELEASE(m_pVertexBuffer);
			}
			...
			void IRenderBorder::Build()
			{
				...
				for(UINT i = 0; i < 4; i++)
				{
					for(UINT j = 0; j < 2; j++)
					{
						SX_SAFE_DELETE_A(pIDXs[i][j]);
					}
					SX_SAFE_DELETE_A(pIDXC[i]);
					SX_SAFE_DELETE_A(pIDXs[i]);
				}
			}
			...
			void IRenderBorder::ReleaseBuffer(buff_t * pIn)
			{
				for(UINT i = 0; i < 2; i++)
				{
					if(pIn->ib)
					{
						SX_SAFE_DELETE_A(pIn->ib[i]);
					}
					if(pIn->vb)
					{
						SX_SAFE_DELETE_A(pIn->vb[i]);
					}
				}
				SX_SAFE_DELETE_A(pIn->ib);
				SX_SAFE_DELETE_A(pIn->vb);
			}
