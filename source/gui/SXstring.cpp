...
/*
DWORD SXStrFind(char* src_str, char *str, DWORD pos)
{
	char *Find = strstr(src_str + pos, str);
...
DWORD SXStrReplace(char* src_str, char *str, char *replace, DWORD pos)
{
	DWORD PosBeginFmt = SXStrFind(src_str, str, pos);
...
DWORD SXStrReplaceAll(char* src_str, char *str, char *replace)
{
	...
	{
		PosCodeBegin = SXStrFind(src_str, str, PosCodeOld);
		if(PosCodeBegin != -1)
		{
			CountCodeFMT++;
	...
	if((PosBeginFmt = SXStrFind(src_str, str, OldPosEndFmt)) != DWORD(-1))
	{
	...
	while((PosBeginFmt = SXStrFind(src_str, str, OldPosEndFmt)) != DWORD(-1))
	{
...
char* SXStrSubStr(char* src_str, DWORD pos, DWORD lenght)
{
...
char* SXStrDelete(char* src_str, DWORD pos, DWORD lenght)
{
	char *str = SXStrSubStr(src_str, pos, lenght);
