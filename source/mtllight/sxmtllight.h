...
SX_LIB_API bool SML_LigthsGetEnable(ID id);				//!< включен ли свет с идентификатором id
...
SX_LIB_API bool SML_LigthsComVisibleForFrustum(ID id, const ISXFrustum* frustum);	//!< виден ли свет в данном фрустуме frustum (отдельный просчет)
SX_LIB_API float SML_LigthsComDistFor(ID id, const float3* viewpos);				//!< дистанция от viewpos до начала света (отдельный просчет)

//! просчет видимости и расстояния от наблюдателя viewpos до начала света, для каждого света (предварительный просчет)
SX_LIB_API void SML_LigthsComVisibleFrustumDistFor(const ISXFrustum* frustum, const float3* viewpos);
...
SX_LIB_API bool SML_LigthsCountUpdateUpdate(
	ID id,					//!< идентификатор света
	const float3* viewpos,	//!< позиция камеры для которой обновляем, на основании расстояния будет произведен расчет обновления
...
SX_LIB_API void SML_LigthsUpdateGFrustums(
	...
	const float3* pos,	//!< позиция наблюдателя
	const float3* dir	//!< направление взгляда наблюдателя
	);
...
SX_LIB_API void SML_LigthsSetShadowBias(ID id, float bias);	//!< установить текущее смещение (bias) при генерации глубины

SX_LIB_API void SML_LigthsSetShadowBlurPixel(ID id, float blur_pixel);	//!< установить размерность размытия (в пикселя) при геренации тени
