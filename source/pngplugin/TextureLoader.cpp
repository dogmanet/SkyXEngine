...
bool XMETHODCALLTYPE CTextureLoader::open(const char *szFileName, const char *szArg)
{
	...
	m_pPngStruct = png_create_read_struct(PNG_LIBPNG_VER_STRING, this, user_error_fn, user_warning_fn);
	if(!m_pPngStruct)
	{
		LibReport(REPORT_MSG_LEVEL_WARNING, "png_create_read_struct() failed '%s'\n", szFileName);
		mem_release(m_pCurrentFile);
		return(false);
	}
	...
	if(!m_pPngInfo)
	{
		LibReport(REPORT_MSG_LEVEL_WARNING, "png_create_info_struct() failed '%s'\n", szFileName);
		mem_release(m_pCurrentFile);
	...
	if(!m_pPngEndInfo)
	{
		LibReport(REPORT_MSG_LEVEL_WARNING, "png_create_info_struct() failed2 '%s'\n", szFileName);
		mem_release(m_pCurrentFile);
