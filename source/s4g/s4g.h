...
//#define S4G_FASTFAIL	//моментальное закрытие приложения во время ошибки
#define S4G_COUNT_TIME_EXPECT_EXIT 50000
//#define S4G_NPRINT_OPS //не выводить команды генерируемые для машины, если объявлен
...
//проверять ли наличие переменной перед ее созданием?
//и если она создана и идет повторное создание то выдать ли ошибку?
//а если переменная не создана и идет обращение к ней выдать ли ошибку?
//при создании переменной обязательно наличие $
//при обращении к переменной обязательно отсутствие $
//для того чтобы это все работало в скриптах надо объявить (в скриптах этот дефайн)
...
S4G_API void s4g_clear(s4g_main* s4gm);//очистка скриптовой системы (для перезагрзуки кода)
