...
s4g_node* s4g_builder_syntax_tree::s4g_get_term()
{
	...
	bst_condition_eof(tmplexs);
		...
		if(tmplexs->type == word_user)
		{
			...
			s4g_node* node = new s4g_node(_var,new s4g_value(t_string,tmplexs->str));
					...
					bst_condition_eof(tmplexs);
						...
						if(tmplexs->type == sym_table_elem && tmplexs->id == 2)
						{
							...
							bst_condition_eof(tmplexs);
								if(tmplexs->type == word_user)
								{
									tmpnode->op1 = new s4g_node(_get,0,new s4g_node(_string,new s4g_value(t_string,tmplexs->str)),new s4g_node(_empty));
		...
		//если тип лексемы строка
			...
			return new s4g_node(_string,new s4g_value(t_string,tmplexs->str));
		//если тип лексемы число с плавающей запятой
		else if(tmplexs->type == word_float)
			return new s4g_node(_float,new s4g_value(t_float,tmplexs->str));
		//если тип лексемы целочисленное значение
		else if(tmplexs->type == word_int)
			return new s4g_node(_int,new s4g_value(t_int,tmplexs->str));
						...
						if(tmplexs->type == sym_arif)
						{
								...
								if(how_type_next == -2)
								{
										...
										else
										{
											...
											stack_var.push(new s4g_node(_numnull,new s4g_value(t_nn)));
										}
								}
			...
			tmplexs = arr_lex->get_curr();
				...
				else if(!(tmplexs->type == sym_delimiter && tmplexs->id == 1))
				{
						...
						if(tmplexs->type == word_user)
						{
								...
								if(node->value == 0)
								{
									tmpnode->value = new s4g_value(t_string,tmplexs->str);
								}
								else
								{
									tmpnode->op1 = new s4g_node(_arg,new s4g_value(t_string,tmplexs->str));
									tmpnode = tmpnode->op1;
								}
