...
void InsertToken(citem_t ** ppFirst, citem_t ** ppLast, token_t tok);
void InsertTokenI(citem_t ** ppFirst, citem_t ** ppLast, token_t tok, int val);
void InsertTokenF(citem_t ** ppFirst, citem_t ** ppLast, token_t tok, float val);

bool tokcmp(const char * pHaystack, const char * pNeedle);

citem_t * Calculate(citem_t * pStart);




void s4g_preprocessor::Define(const char * def, int len, bool bTemp)
{
	if(len == 0)
	{
		len = strlen(def);
...
void s4g_preprocessor::Undef(const String & def)
{
...
void s4g_preprocessor::AddIncPath(const String & s)
{
	m_vIncludes.push_back({s});
}

void s4g_preprocessor::UndefTemp()
{
...
String s4g_preprocessor::Process(const char * src, const char * file)
{
		...
		if(!isspace(0xFF & ch) || !sNewLine)
		{
			...
			if((ch == '\n' || ch == '\r') && cp != '\\')
			{
				...
				if(sDirective) // parse directive
				{
					...
					{
						String path;
						String inc = GetInclude(dArgs.substr(1, dArgs.length() - 2), &path);
	...
	if(out.size())
	{
		_out.Reserve(out.size());
		memcpy((void*)(_out.c_str()), &(out[0]), out.size() * sizeof(char));
		((char*)(_out.c_str()))[out.size()] = 0;
	}
...
bool s4g_preprocessor::isDefined(const String & def)
{
	if(!m_mDefs.KeyExists(def))
	{
		return(false);
...
bool s4g_preprocessor::isTrue(const String & _expr)
{
...
String s4g_preprocessor::MakeExpansion(const String & expr)
{
	ReplList rlist;
	return(MakeExpansion(expr, rlist));
}

String s4g_preprocessor::MakeExpansion(const String & _expr, ReplList & rlist)
{
	String tok;
	String expr = _expr;
...
String s4g_preprocessor::ReplArgs(const String & _expr, const _define * pDef, const Array<String> & szArgs)
{
	String tok, tok_prev;
	String expr = _expr;
	if(!isspace(0xFF & _expr[_expr.length() - 1]))
...
String s4g_preprocessor::CheckDefines(const String & _expr)
{
	String expr = _expr;
	UINT deflen = strlen("defined");
	bool sDFound = false;
	bool sPFound = false;
	String key;
	UINT sPos = ~0;
...
String s4g_preprocessor::GetInclude(const String & name, String * outPath)
{
...
const char * s4g_preprocessor::GetError()
{
	return(m_sError.c_str());
}
bool s4g_preprocessor::IsError()
{
...
String s4g_preprocessor::Stringify(const String & expr)
{
	return(String("\"") + expr + "\"");
}