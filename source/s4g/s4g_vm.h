...
enum s4g_vm_command
{
	...
	mc_mul,			//-1 * 0
	mc_div,			//-1 / 0
	mc_preincr,		//преинкремент ++val
	mc_predecr,		//предекремент -- val
	mc_postincr,	//постинкремент val++
	mc_postdecr,	//постдекремент val++
	...
	mc_block_new,	//создание нового окружения для блока выражений
	mc_block_del,	//удаление окружения блока выражений
	...
	mc_bit_shiftr,
	mc_bit_shiftl,
	mc_log_neqt,// !== (неравенство без приведения типов)
	mc_log_eq,	// ==
	mc_log_neq,	// !=
	mc_log_ge,	// >=
	mc_log_le,	// <=
	...
	mc_log_lt,	// <
	mc_log_not,	// !(expr)
	mc_log_eqt,	// === (равенство без приведения типов)
	mc_bit_and,
	mc_bit_or,
