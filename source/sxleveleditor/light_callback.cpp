...
{
	if (type == LightsTypeLight::ltl_global)
	{
	...
	else if (type == LightsTypeLight::ltl_point)
	{
	...
	else if (type == LightsTypeLight::ltl_direction)
	{
		LCActivateGlobal(false);
		LCActivatePoint(false);
...
void LCInitElemsSelModel(int sel)
{
	if (sel >= 0 && sel < SML_LigthsGetCount())
	{
		...
		if (SML_LigthsGetType(sel) == LightsTypeLight::ltl_global)
			LCActivateGlobal(true);
		else if (SML_LigthsGetType(sel) == LightsTypeLight::ltl_point)
			LCActivatePoint(true);
		else if (SML_LigthsGetType(sel) == LightsTypeLight::ltl_direction)
			LCActivateDirect(true);
		...
		SXLevelEditor::LightCheckBoxGlobal->SetCheck(SML_LigthsGetType(sel) == LightsTypeLight::ltl_global);
		SXLevelEditor::LightComboBoxTypeLight->SetSel((SML_LigthsGetType(sel) == LightsTypeLight::ltl_global || SML_LigthsGetType(sel) == LightsTypeLight::ltl_point) ? 0 : 1);
		...
		float3 pos;
		SML_LigthsGetPos(sel, &pos, false, false/*SML_LigthsGetType(sel) == LIGHTS_TYPE_GLOBAL*/);
...
LRESULT SXLevelEditor_LightButtonFinish_Click(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	if (tmpid >= 0)
	{
		if (SML_LigthsGetType(SML_LigthsGetIDOfKey(tmpid)) == LightsTypeLight::ltl_global)
		{
			for (int i = 0; i < 5; ++i)
			{
		...
		else if (SML_LigthsGetType(SML_LigthsGetIDOfKey(tmpid)) == LightsTypeLight::ltl_point)
		{
			for (int i = 0; i < 6; ++i)
			{
				SGeom_ModelsDelArrForCom(SML_LigthsGetIDArr(SML_LigthsGetIDOfKey(tmpid), 0, i));
		...
		else if (SML_LigthsGetType(SML_LigthsGetIDOfKey(tmpid)) == LightsTypeLight::ltl_direction)
		{
	...
	if (typelight == 0)
	{
		if (!SXLevelEditor::LightCheckBoxGlobal->GetCheck())
		{
			tmpid = SML_LigthsReCreatePoint((tmpid == -1 ? tmpid : SML_LigthsGetIDOfKey(tmpid)),
				&tmppos,
				power,
				radius_height,
				&tmpcolor,
	...
	{
		tmpid = SML_LigthsReCreateDirection((tmpid == -1 ? tmpid : SML_LigthsGetIDOfKey(tmpid)),
			&tmppos,
			power,
			radius_height,
			&tmpcolor,
	...
	if (!SXLevelEditor::LightCheckBoxGlobal->GetCheck())
	{
		SML_LigthsSetTypeShadowed(tmpid, (LightsTypeShadow)SXLevelEditor::LightComboBoxTypeLightShadow->GetSel());
