...
void SXUpdateParam::ColorText()
{
		if(SXMainWndElem::NumActiveElement != -1)
		{
			...
			SXColor color = SXMainWndElem::CreateElements[SXMainWndElem::NumActiveElement]->Object->GetColorText();
			sprintf(bufr,"%d",color.GetR());
			sprintf(bufg,"%d",color.GetG());
			sprintf(bufb,"%d",color.GetB());
...
void SXUpdateParam::ColorTextBK()
{
		...
		{
			char bufr[16],bufg[16],bufb[16];
			SXColor color = SXMainWndElem::CreateElements[SXMainWndElem::NumActiveElement]->Object->GetColorTextBk();
			sprintf(bufr,"%d",color.GetR());
			sprintf(bufg,"%d",color.GetG());
			sprintf(bufb,"%d",color.GetB());
...
void SXUpdateParam::ColorBK()
{
	...
	SXColor color;
		...
		{
			SXMainWndElem::JobMainWnd->GetColorBrush(&color);
		}
	sprintf(bufr,"%d",color.GetR());
	sprintf(bufg,"%d",color.GetG());
	sprintf(bufb,"%d",color.GetB());
	...
	sscanf(bufb,"%d",&numb);
		...
		if(SXMainWndElem::NumActiveElement != -1)
		{
				if(strcmp(SXMainWndElem::CreateElements[SXMainWndElem::NumActiveElement]->SXClassName,"SXGUIBaseWnd") == 0)
					((SXGUIBaseWnd*)SXMainWndElem::CreateElements[SXMainWndElem::NumActiveElement]->Object)->SetColorBrush(numr,numg,numb);
				else
...
void SXUpdateParam::SetFont()
{
	...
	SXMainWndElem::WndLog->Enable(false);
		if(ChooseFont(&cf))
		{
			...
			sprintf(buf,"%s%s%d",lf.lfFaceName,", ",font_size);
				if(SXMainWndElem::NumActiveElement != -1)
					SXMainWndElem::CreateElements[SXMainWndElem::NumActiveElement]->Object->SetFont(lf.lfFaceName,font_size,lf.lfWidth,lf.lfWeight,lf.lfItalic,lf.lfUnderline,lf.lfStrikeOut);
			SXMainWndElem::ButtonParamSelectFont->SetText(buf);
			SXUpdateParam::ParentFont();
		}
...
void SXUpdateParam::SetImgButton()
{
	...
	SXMainWndElem::WndLog->Enable(false);
		if(GetOpenFileName(&ofn) == TRUE)
		{
				...
				{
					SXGUIButtonImg* bimg = (SXGUIButtonImg*)SXMainWndElem::CreateElements[SXMainWndElem::NumActiveElement]->Object;
...
void SaveFile(const char* path)
{
	...
	//{
	SXMainWndElem::JobMainWnd->GetColorBrush(&SXColorText);
	ColorBK = SXColorText.GetColor();
	...
	//char SXClassName[32];
		for(int i=0;i<CountSave;i++)
		{
			...
			CaptionElement[0] = 0;
				...
				{*/
					SXGUITextual* textual = (SXGUITextual*)SXMainWndElem::CreateElements[i]->Object;
			...
			ColorText = ColorTextBK = ColorBK = 0;
				...
				{
					SXColorText = SXMainWndElem::CreateElements[i]->Object->GetColorText();
					SXColorTextBK = SXMainWndElem::CreateElements[i]->Object->GetColorTextBk();
			...
			//{
				...
				{
					SXColorBK = SXMainWndElem::CreateElements[i]->Object->GetColorBrush();
			...
			fwrite(&Enable,sizeof(bool),1,file);
				...
				if(strcmp(SXMainWndElem::CreateElements[i]->SXClassName,"SXGUIButtonImg") == 0)
				{
					...
					PathForImg[0] = 0;
					SXGUIButtonImg* tmpImgButton = (SXGUIButtonImg*)SXMainWndElem::CreateElements[i]->Object;
...
void LoadFile(const char* path)
{
	...
	SXMainWndElem::JobMainWnd->SetColorBrush(SXColorText.GetR(),SXColorText.GetG(),SXColorText.GetB());
	...
	int CountObject = 0;
		for(int i=0;i<CountSave;i++)
		{
			...
			//}
				...
				{
					CountObject = 0;
					Object->Object = new SXGUIButton(CaptionElement,GClientRect.left,GClientRect.top,Width,Height,0,SXMainWndElem::JobMainWnd->GetHWND(),WndProcChildJob,0);
				}
				else if(strcmp(Object->SXClassName,"SXGUIButtonImg") == 0)
				{
					CountObject = 1;
					SXGUIButtonImg* tmpButtonImg = new SXGUIButtonImg("",GClientRect.left,GClientRect.top,Width,Height,RGB(0,0,0),RGB(100,100,100),SXMainWndElem::JobMainWnd->GetHWND(),WndProcChildJob,0);
					tmpButtonImg->FrameColor = SXColor(100,100,100);
				...
				{
					CountObject = 2;
					Object->Object = new SXGUIEdit(CaptionElement,GClientRect.left,GClientRect.top,Width,Height,SXMainWndElem::JobMainWnd->GetHWND(),WndProcChildJob,0);
				}
				else if(strcmp(Object->SXClassName,"SXGUICheckBox") == 0)
				{
					CountObject = 3;
					Object->Object = new SXGUICheckBox(CaptionElement,GClientRect.left,GClientRect.top,Width,Height,SXMainWndElem::JobMainWnd->GetHWND(),WndProcChildJob,0,true);
				}
				else if(strcmp(Object->SXClassName,"SXGUIComboBox") == 0)
				{
					CountObject = 4;
					Object->Object = new SXGUIComboBox(CaptionElement,GClientRect.left,GClientRect.top,Width,Height,SXMainWndElem::JobMainWnd->GetHWND(),WndProcChildJob,0);
				}
				else if(strcmp(Object->SXClassName,"SXGUIGroupBox") == 0)
				{
					CountObject = 5;
					Object->Object = new SXGUIGroupBox(CaptionElement,GClientRect.left,GClientRect.top,Width,Height,SXMainWndElem::JobMainWnd->GetHWND(),WndProcChildJob,0);
				}
				else if(strcmp(Object->SXClassName,"SXGUIListBox") == 0)
				{
					CountObject = 6;
					Object->Object = new SXGUIListBox(CaptionElement,GClientRect.left,GClientRect.top,Width,Height,SXMainWndElem::JobMainWnd->GetHWND(),WndProcChildJob,0,true);
					SendMessage(Object->Object->GetHWND(),LB_SETITEMHEIGHT,0,1);
				}
				else if(strcmp(Object->SXClassName,"SXGUIMemo") == 0)
				{
					CountObject = 7;
					Object->Object = new SXGUIMemo(CaptionElement,GClientRect.left,GClientRect.top,Width,Height,SXMainWndElem::JobMainWnd->GetHWND(),WndProcChildJob,0);
				}
				...
				{
					CountObject = 9;
					Object->Object = new SXGUIRadioButton(CaptionElement,GClientRect.left,GClientRect.top,Width,Height,SXMainWndElem::JobMainWnd->GetHWND(),WndProcChildJob,0);
				}
				else if(strcmp(Object->SXClassName,"SXGUIStatic") == 0)
				{
					CountObject = 10;
					Object->Object = new SXGUIStatic(CaptionElement,GClientRect.left,GClientRect.top,Width,Height,SXMainWndElem::JobMainWnd->GetHWND(),WndProcChildJob,0);
				}
				else if(strcmp(Object->SXClassName,"SXGUITrackBar") == 0)
				{
					CountObject = 11;
					Object->Object = new SXGUITrackBar(CaptionElement,GClientRect.left,GClientRect.top,Width,Height,SXMainWndElem::JobMainWnd->GetHWND(),WndProcChildJob,0);
				}
				else if(strcmp(Object->SXClassName,"SXGUIStatusBar") == 0)
				{
					CountObject = 12;
					Object->Object = new SXGUIStatusBar(CaptionElement,SXMainWndElem::JobMainWnd->GetHWND(),WndProcChildJob,0);
				}
				else if(strcmp(Object->SXClassName,"SXGUIToolBar") == 0)
				{
					CountObject = 13;
					Object->Object = new SXGUIToolBar(CaptionElement,GClientRect.left,GClientRect.top,Width,Height,Width,Height,SXMainWndElem::JobMainWnd->GetHWND(),WndProcChildJob,0);
				}
				...
				{
					CountObject = 15;
					Object->Object = new SXGUIBaseWnd(Object->Name,CaptionElement,0,
			...
			ColorText = ColorTextBK = ColorBK = 0;
					...
					)
				{
					...
					Object->Object->SetColorText(SXColorText.GetR(),SXColorText.GetG(),SXColorText.GetB());
					Object->Object->SetColorTextBk(SXColorTextBK.GetR(),SXColorTextBK.GetG(),SXColorTextBK.GetB());
			...
			//{
					...
					)
				{
					...
					Object->Object->SetColorBrush(SXColorBK.GetR(),SXColorBK.GetG(),SXColorBK.GetB());
				}
			//}
			...
			Object->Object->Enable(Enable);
				...
				if(strcmp(Object->SXClassName,"SXGUIButtonImg") == 0)
				{
					...
					fseek(file,1,SEEK_CUR);
					SXGUIButtonImg* tmpImgButton = (SXGUIButtonImg*)Object->Object;
			...
			InLog("%s%s%s%s%s","Создан элемент sx_class [",Object->SXClassName,"], sys_class [",Object->SysClassName,"]");
			...
			InLog("%s","Параметры ParamWnd перезаписаны, порядковый номер элемента записан (userdata32)");
		}
...
{
	InLog("%s","Начинаем все удалять и стартовать новый проект ...");
	SXMainWndElem::NumActiveElement = -1;
	int size = SXMainWndElem::CreateElements.size();
		for(int i=0;i<size;i++)
		{
			InLog("%s%s%s","Удаление объекта [",SXMainWndElem::CreateElements[0]->Name,"] ...");
			...
			InLog("%s","Удаление объекта завершено");
		}
	InLog("%s","Очистка закончена, новый проект готов!");
...
void OutputInFile(const char* path,const char* name_space,bool in_ns,bool constructors,bool destructors,bool comments,bool include)
{
	...
	fprintf(file,"%s","\n");
		...
		if(constructors)
		{
			...
			SXGUIBaseWnd* tmpwnd = (SXGUIBaseWnd*)SXMainWndElem::JobMainWnd;
			SXColor SXColorBK;
			tmpwnd->GetColorBrush(&SXColorBK);
			fprintf(file,"%s%d%s%d%s%d%s","CreateSolidBrush(RGB(",SXColorBK.GetR(),",",SXColorBK.GetG(),",",SXColorBK.GetB(),")),"); //CreateSolidBrush(RGB(200,200,200))
			...
			char CaptionElement[256];
				for(int i=0;i<SXMainWndElem::CreateElements.size();i++)
				{
					...
					Height = GClientRect.bottom - GClientRect.top;
						...
						{
							SXGUIButtonImg* tmpButtonImg = (SXGUIButtonImg*)SXMainWndElem::CreateElements[i]->Object;
							char path[1024];
							...
							fprintf(file,"%s%s%s%s","\t",NameSpacePP,SXMainWndElem::CreateElements[i]->Name,"->FrameColor = SXColor(100,100,100);\n");
						...
						else if(strcmp(SXMainWndElem::CreateElements[i]->SXClassName,"SXGUIBaseWnd") == 0)
						{
							...
							SXGUIBaseWnd* tmpwnd = (SXGUIBaseWnd*)SXMainWndElem::CreateElements[i]->Object;
							SXColor SXColorBK;
							tmpwnd->GetColorBrush(&SXColorBK);
							fprintf(file,"%s%d%s%d%s%d%s","CreateSolidBrush(RGB(",SXColorBK.GetR(),",",SXColorBK.GetR(),",",SXColorBK.GetR(),")),"); //CreateSolidBrush(RGB(200,200,200))
						...
						{
							SXColor SXColorText = SXMainWndElem::CreateElements[i]->Object->GetColorText();
							SXColor SXColorTextBK = SXMainWndElem::CreateElements[i]->Object->GetColorTextBk();

							bool TransparentTextBK = SXMainWndElem::CreateElements[i]->Object->GetTransparentTextBk();

							fprintf(file,"%s%s%s%s%d%s%d%s%d%s","\t",NameSpacePP,SXMainWndElem::CreateElements[i]->Name,"->SetColorText(",SXColorText.GetR(),",",SXColorText.GetG(),",",SXColorText.GetB(),");\n");
							fprintf(file,"%s%s%s%s%d%s%d%s%d%s","\t",NameSpacePP,SXMainWndElem::CreateElements[i]->Name,"->SetColorTextBk(",SXColorTextBK.GetR(),",",SXColorTextBK.GetG(),",",SXColorTextBK.GetB(),");\n");
					...
					//{
						...
						{
							SXColor SXColorBrush = SXMainWndElem::CreateElements[i]->Object->GetColorBrush();
							fprintf(file,"%s%s%s%s%d%s%d%s%d%s","\t",NameSpacePP,SXMainWndElem::CreateElements[i]->Name,"->SetColorBrush(",SXColorBrush.GetR(),",",SXColorBrush.GetG(),",",SXColorBrush.GetB(),");\n");
						}
					//}
